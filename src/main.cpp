...
// EEPROM variables
const byte saveUnits = 0;
const byte saveAlertThreshold = 1; // Addresses for storing settings data in the EEPROM
const byte saveCalibration = 2;
const byte saveDeviceMode = 3;
const byte saveLoggingMode = 4;
const byte saveSSIDLen = 5;
const byte savePWLen = 6;
const byte saveIDLen = 7;
const byte saveAPILen = 8;
...
// void initArray(char a[], int st, int size) {
//   for (int i = st; i < st + size; i++)
//   {
//     a[i - st] = EEPROM.read(i);
//   }
// }
void setCurPrint(byte x, byte y, char msg[]) {
  tft.setCursor(x, y);
  tft.println(msg);
}
void checkLogging(boolean isLogging) {
  if (isLogging)
  {
    tft.fillRoundRect(3, 114, 234, 44, 4, 0x3B8F);
    tft.drawRoundRect(3, 114, 234, 44, 4, WHITE);
    setCurPrint(38,145,"UPLOAD DATA");
  }
  else
  {
    tft.fillRoundRect(3, 114, 234, 44, 4, 0xB9C7);
    tft.drawRoundRect(3, 114, 234, 44, 4, WHITE);
    setCurPrint(33,145,"LOGGING OFF");
  }
}
...
void setup()
{
  ...
  for (int i = 10; i < 10 + EEPROM.read(saveSSIDLen); i++)
  {
    ssid[i - 10] = EEPROM.read(i);
  }

  for (int j = 30; j < 30 + EEPROM.read(savePWLen); j++)
  {
    password[j - 30] = EEPROM.read(j);
  }

  for (int k = 50; k < 50 + EEPROM.read(saveIDLen); k++)
  {
    channelID[k - 50] = EEPROM.read(k);
  }

  for (int l = 70; l < 70 + EEPROM.read(savePWLen); l++)
  {
  ...
  else
  {
    ...
    setCurPrint(38, 140, "Connecting to WiFi..");
    ...
    {
      deviceMode = 0; 
      setCurPrint(45, 200, "Failed to connect.");
      delay(1000);
    }
    else
    {
      setCurPrint(68, 200, "Connected!");
      delay(1000);
    }
    drawHomePage();
  }
    ...
    if (ts.touched() && !wasTouched) // A way of "debouncing" the touchscreen. Prevents multiple inputs from single touch
    {
      ...
      if ((x > 162 && x < 238) && (y > 259 && y < 318))
      {
        ...
        if (integrationMode == 0) // change button based on touch and previous state
        {
          ...
          tft.setTextSize(1);
          setCurPrint(180, 283, "INT");
          setCurPrint(177, 309, "60 s");
        }
        else if (integrationMode == 1)
        {
          ...
          tft.setFont(&FreeSans12pt7b);
          tft.setTextSize(1);
          setCurPrint(180, 283, "Connecting to WiFi..");
          setCurPrint(184, 309, "5 s");
        }
  ...
  else if (page == 2) // units page
  {
    ...
    if (ts.touched() && !wasTouched)
    {
      ...
      else if ((x > 4 && x < 234) && (y > 70 && y < 120))
      {
        ...
        setCurPrint(30,103,"Sieverts (uSv/hr)");

        tft.fillRoundRect(4, 128, 232, 48, 4, ILI9341_BLACK);
        setCurPrint(47,160,"Rems (mR/hr)");
      }
  ...
  else if (page == 5)  // Wifi page
  {
    ...
    if (ts.touched() && !wasTouched)
    {
      ...
      else if ((x > 3 && x < 237) && (y > 64 && y < 108))  // wifi setup button
      {
        ...
        setCurPrint(50,50,"AP SETUP MODE");
        tft.drawFastHLine(50, 53, 145, ILI9341_WHITE);
        byte yA[] = {80,100,120,140,160,180,200,220,240};
        char* msg[] = {"With any WiFi capable","device, connect to","network \"GC20\" and ",
        "browse to 192.168.4.1","Enter credentials","of your WiFi network","and the Channel ID and","write API key of your","ThingSpeak channel"};
        for(byte ix=0; ix < 9; ix++) {
          setCurPrint(20,yA[ix],msg[ix]);          
        }
        ...
        setCurPrint(16,265,"Settings saved. Restarting");
      ...
      else if ((x > 3 && x < 237) && (y > 162 && y < 206)) // upload data
      {
        ...
        drawBlankDialogueBox();
        setCurPrint(16,265,"Connecting to Wifi..");
        delay(100);
        ...
        if (client.connect(server, 80)) {          // post data to thingspeak
          ...
          setCurPrint(43,260,"Resetting Device..");
          delay(1000);
          ESP.reset();                 
        }
        else 
        {
          setCurPrint(50,260,"Failed to upload");
          delay(1000);
          ESP.reset();
        }
  ...
  else if (page == 8)          // device mode selection page
  {
    ...
    if (ts.touched() && !wasTouched)
    {
      ...
      else if ((x > 4 && x < 234) && (y > 70 && y < 120))
      {
        ...
        tft.setFont(&FreeSans12pt7b);
        tft.fillRoundRect(4, 71, 232, 48, 4, 0x2A86);
        setCurPrint(13,103,"GEIGER COUNTER");

        tft.fillRoundRect(4, 128, 232, 48, 4, ILI9341_BLACK);
        setCurPrint(30,160,"MON. STATION");
      }
      else if ((x > 4 && x < 234) && (y > 127 && y < 177))
      {
        deviceMode = 1;
        ...
        tft.fillRoundRect(4, 128, 232, 48, 4, 0x2A86);
        setCurPrint(38,145,"MON. STATION");
      }
    }
  }
}
...
void drawHomePage()
{
  ...
  setCurPrint(16,40,"EFFECTIVE DOSE RATE:");
  ...
  tft.setFont(&FreeSans12pt7b);
  setCurPrint(7,141,"CPM:");
  tft.drawRoundRect(3, 117, 234, 32, 3, DOSEBACKGROUND);

  tft.fillRoundRect(3, 151, 185, 105, 4, 0x630C);
  tft.setFont(&FreeSans9pt7b);
  setCurPrint(9,171,"CUMULATIVE DOSE");
  setCurPrint(7,205,"Counts:");
  if (doseUnits == 0)
  {
    setCurPrint(34,235,"uSv:");
  }
  else if (doseUnits == 1)
  {
    setCurPrint(37,235,"mR:");
  }
  ...
  tft.setFont(&FreeSans12pt7b);
  tft.setTextSize(1);
  setCurPrint(74,284,"TIMED");
  setCurPrint(70,309,"COUNT");
...
void drawAlertPage()
{
  ...
  tft.fillRoundRect(3, 23, 234, 40, 3, 0x3B8F);
  tft.setFont(&FreeSans12pt7b);
  setCurPrint(4,51,"ALERT THRESHOLD");
  tft.drawFastHLine(5, 55, 229, WHITE);

  drawBackButton();
  
  setCurPrint(30,164,"uSv/hr:");
...
void drawCalibrationPage()
{
  ...
  tft.fillRoundRect(3, 23, 234, 40, 3, 0x3B8F);
  tft.setFont(&FreeSans12pt7b);
  setCurPrint(47,51,"CALIBRATE");
  tft.drawFastHLine(48, 55, 133, WHITE);

  drawBackButton();

  tft.setFont(&FreeSans9pt7b);
  setCurPrint(8,154,"Conversion Factor");
  setCurPrint(8,174,"(CPM per uSv/hr)");
...
void drawWifiPage()
{
  ...
  tft.fillRoundRect(3, 23, 234, 35, 3, 0x3B8F);
  tft.setFont(&FreeSans12pt7b);
  setCurPrint(7,48,"LOGGING AND WIFI");
  tft.drawFastHLine(8, 51, 222, WHITE);

  tft.fillRoundRect(3, 64, 234, 44, 4, 0x2A86);
  tft.drawRoundRect(3, 64, 234, 44, 4, WHITE);
  setCurPrint(48,94,"WIFI SETUP");

  checkLogging(isLogging);
  
  tft.fillRoundRect(3, 164, 234, 44, 4, 0x2A86);
  tft.drawRoundRect(3, 164, 234, 44, 4, WHITE);
  setCurPrint(31,194,"UPLOAD DATA");

  tft.fillRoundRect(3, 214, 234, 44, 4, 0x2A86);
  tft.drawRoundRect(3, 214, 234, 44, 4, WHITE);
  setCurPrint(35,244,"DEVICE MODE");

  if (addr > 2000)
  {
    tft.setFont(&FreeSans9pt7b);
    setCurPrint(80,297,"Log memory full");
  }
}
...
void drawTimedCountPage()
{
  ...
  tft.setFont(&FreeSans12pt7b);
  setCurPrint(149,302,"BEGIN!");

  tft.fillRoundRect(3, 23, 234, 40, 3, 0x3B8F);
  setCurPrint(34,51,"TIMED COUNT");
  tft.drawFastHLine(35, 55, 163, WHITE);

  tft.setFont(&FreeSans9pt7b);
  setCurPrint(5,162,"Duration (minutes):");
...
void drawTimedCountRunningPage(int duration, int size)
{
  ...
  setCurPrint(58,90,"Progress:");
  setCurPrint(13,150,"Duration:");
  tft.setCursor(115, 150);
  tft.println(duration);
  setCurPrint((135 + (size - 1)*15),150,"min");
  setCurPrint(15,200,"Counts:");
  setCurPrint(37,245,"CPM:");
...
void drawDeviceModePage()
{
  ...
  tft.setFont(&FreeSans9pt7b);
  setCurPrint(20,200,"Press Back button and");
  setCurPrint(20,220,"reset device for changes");
  setCurPrint(20,240,"to take effect");
}
